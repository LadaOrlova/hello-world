...
Дата: 2026-03-01
Версия: 2.0 (финальная, с исправлениями по кросс-проверке)
...
Помимо единого `aha_moment_reached`, для каждого сегмента определён СВОЙ Aha-момент, который трекается отдельным событием `aha_segment_reached`:

| Сегмент | Aha-момент | Триггерные действия | Модуль |
|---|---|---|---|
| Non-Technical Founders | "У меня есть PRD + лендинг + понимание клиента — за 2 часа!" | exercise_completed (describe_product_jobs) + prompt_result_saved (idea_validator) | Module 1 |
| Solopreneurs | "За 30 минут понял, жизнеспособна ли идея" | prompt_result_saved (idea_validator) + > 30 сек на результате (быстрая оценка идеи) | Module 1 |
| AI-Era Career Builders | "Могу системно проанализировать любой продукт" | exercise_completed в Module 4 (граф работ) + 3+ промптов использовано | Module 4 |
| AI Skills Seekers | "Создал работающий лендинг/прототип одним промптом" | skill_downloaded + первый успешный запуск (Module 5) | Module 5 |

**Стратегия:** Aha-момент сдвинут максимально влево — в бесплатный Module 1 для приоритетных сегментов (Founders, Solopreneurs). Для вторичных сегментов (Career Builders, AI Skills Seekers) Aha наступает позже, что означает необходимость более сильной мотивации для прохождения до платных модулей.
...
| `onboarding_has_idea` | Ответил на вопрос "Do you have a product idea?" | При выборе ответа в онбординге | `has_idea` (boolean), `segment` | funnel, cohort |
...
| `aha_moment_reached` | Достиг общего Aha-момента | При выполнении обоих условий: exercise_completed (describe_product_jobs) + prompt_result_saved (idea_validator) + > 30 сек на результате | `segment`, `time_from_signup_seconds`, `trigger_exercise`, `trigger_prompt` | funnel, cohort |
| `aha_segment_reached` | Достиг сегмент-специфичного Aha-момента | При выполнении условий Aha для конкретного сегмента (см. таблицу выше) | `segment`, `aha_type` (founders / solopreneurs / career / seekers), `trigger_action`, `time_to_aha_seconds`, `module` | funnel, cohort |
...
### Course Content Events — Артефакты

| Название события | Описание | Когда трекать | Properties | Тип |
|---|---|---|---|---|
| `artifact_created` | Студент создал артефакт (лендинг, PRD, план воронки, анализ сегментов) | При успешном создании артефакта через Skill или промпт | `type` (landing_page / prd / funnel_plan / segment_analysis), `module`, `tool_used` (claude_code_skill / web_prompt / manual), `segment` | funnel, cohort |
...
### Cross-sell & Upsell Events

| Название события | Описание | Когда трекать | Properties | Тип |
|---|---|---|---|---|
| `cross_sell_shown` | Показано предложение cross-sell продукта | При показе предложения на UI | `product_name` (templates_pack / certificate / community_pro / prompt_pack_gtm / prd_from_interview), `price`, `currency`, `trigger`, `user_plan` | revenue |
| `cross_sell_purchased` | Cross-sell продукт куплен | При успешной оплате cross-sell | `product_name`, `price`, `currency`, `payment_method` | revenue |
| `upsell_shown` | Показано предложение upsell продукта | При показе upsell на UI | `product_name` (advanced_course / product_review / cohort_bootcamp), `price`, `currency`, `trigger` | revenue |
| `upsell_purchased` | Upsell продукт куплен | При успешной оплате upsell | `product_name`, `price`, `currency` | revenue |
| `subscription_started` | Подписка оформлена (Community Pro) | При успешной оплате подписки | `plan` (community_pro), `price`, `currency`, `billing_period` (monthly) | revenue |
| `subscription_cancelled` | Подписка отменена | При отмене подписки пользователем | `plan`, `reason`, `lifetime_months` | revenue, churn |

### Exit Survey Events

| Название события | Описание | Когда трекать | Properties | Тип |
|---|---|---|---|---|
| `exit_survey_completed` | Free user заполнил exit-опрос о причинах неконверсии | Через 14 дней после signup, если пользователь не купил курс | `days_since_signup`, `modules_completed`, `aha_reached`, `reason` (no_need_product_thinking / no_need_vibe_coding / too_expensive / dont_trust_instructor / not_relevant / other), `reason_text`, `segment` | churn, cohort |

### Prompt Health Events

| Название события | Описание | Когда трекать | Properties | Тип |
|---|---|---|---|---|
| `prompt_health_check` | Автоматическая проверка работоспособности промпта (серверное) | При ежесуточной автоматической проверке каждого промпта | `prompt_name`, `status` (pass / fail), `response_format_valid` (boolean), `response_time_ms`, `ai_model` | monitoring |
...
function identifyUser(
  ...
  posthog.identify(userId, {
    ...
    aha_segment_reached: false,
    course_completed: false,
    community_joined: false,
    nps_score: null,
    has_product_idea: null, // [ИСПРАВЛЕНИЕ 4] — заполняется при onboarding_has_idea
    artifacts_created_count: 0, // [ИСПРАВЛЕНИЕ 4] — инкрементируется при artifact_created
    cross_sell_purchased_count: 0, // [ИСПРАВЛЕНИЕ 4] — инкрементируется при cross_sell_purchased
...
// posthog.people.increment('artifacts_created_count', 1)

// При cross-sell покупке:
// posthog.people.increment('cross_sell_purchased_count', 1)
```
...
// === onboarding_has_idea === [ИСПРАВЛЕНИЕ 4]
// Вызывать при ответе на вопрос "Do you have a product idea?"
function trackOnboardingHasIdea(hasIdea: boolean, userSegment: string): void {
  posthog.capture('onboarding_has_idea', {
    has_idea: hasIdea,
    segment: userSegment,
  })

  updateUserProperties({ has_product_idea: hasIdea })
}
...
// === aha_segment_reached === [ИСПРАВЛЕНИЕ 1]
// Сегмент-специфичные Aha-моменты
// Вызывать при выполнении условий Aha для конкретного сегмента:
// - Founders: exercise_completed (describe_product_jobs) + prompt_result_saved (idea_validator)
// - Solopreneurs: prompt_result_saved (idea_validator) + > 30 сек на результате
// - Career Builders: exercise_completed в Module 4 (граф работ) + 3+ промптов использовано
// - AI Skills Seekers: skill_downloaded + первый успешный запуск (Module 5)
function trackAhaSegmentReached(
  userSegment: string,
  ahaType: 'founders' | 'solopreneurs' | 'career' | 'seekers',
  triggerAction: string,
  module: string
): void {
  const timeSinceSignup = getTimeFromSignupSeconds()

  posthog.capture('aha_segment_reached', {
    segment: userSegment,
    aha_type: ahaType,
  ...
  updateUserProperties({
    aha_segment_reached: true,
    aha_segment_type: ahaType,
    aha_segment_reached_at: new Date().toISOString(),
  })
}
```
...
### Course Content Events — Артефакты [ИСПРАВЛЕНИЕ 4]

```typescript
// === artifact_created ===
// Вызывать когда студент создал артефакт (лендинг, PRD и т.д.)
function trackArtifactCreated(
  type: 'landing_page' | 'prd' | 'funnel_plan' | 'segment_analysis',
  module: string,
  toolUsed: 'claude_code_skill' | 'web_prompt' | 'manual',
  userSegment: string
): void {
...
### Cross-sell & Upsell Events [ИСПРАВЛЕНИЕ 2]

```typescript
// === cross_sell_shown ===
// Вызывать при показе cross-sell предложения
function trackCrossSellShown(
  productName: 'templates_pack' | 'certificate' | 'community_pro' | 'prompt_pack_gtm' | 'prd_from_interview',
  price: number,
  currency: string,
  trigger: string,
  userPlan: string
): void {
  posthog.capture('cross_sell_shown', {
    product_name: productName,
    price,
    currency,
    trigger, // 'module_5_completed' | 'course_completed' | 'dashboard' | 'email_upsell'
    user_plan: userPlan, // 'standard' | 'premium'
  })
}

// === cross_sell_purchased ===
// Вызывать при успешной покупке cross-sell продукта
function trackCrossSellPurchased(
  productName: 'templates_pack' | 'certificate' | 'community_pro' | 'prompt_pack_gtm' | 'prd_from_interview',
  price: number,
  currency: string,
  paymentMethod: string
): void {
  posthog.capture('cross_sell_purchased', {
    product_name: productName,
    price,
    currency,
...
// === upsell_shown ===
// Вызывать при показе upsell предложения
function trackUpsellShown(
  productName: 'advanced_course' | 'product_review' | 'cohort_bootcamp',
  price: number,
  currency: string,
  trigger: string
): void {
  posthog.capture('upsell_shown', {
    product_name: productName,
    price,
    currency,
    trigger, // 'course_completed' | 'module_6_completed' | 'email_upsell'
  })
}

// === upsell_purchased ===
// Вызывать при успешной покупке upsell продукта
function trackUpsellPurchased(
  productName: 'advanced_course' | 'product_review' | 'cohort_bootcamp',
  price: number,
  currency: string
): void {
  posthog.capture('upsell_purchased', {
    product_name: productName,
    price,
    currency,
  })
...
// Вызывать при оформлении подписки (Community Pro)
...
function trackSubscriptionCancelled(
  ...
  posthog.capture('subscription_cancelled', {
    plan,
    reason: userReason,
    lifetime_months: monthsSinceStart,
  })
}
```

### Exit Survey Events [ИСПРАВЛЕНИЕ 3]

```typescript
// === exit_survey_completed ===
// Вызывать когда free user, не купивший через 14 дней, заполнил exit-опрос
// Триггер: серверный cron определяет пользователей (is_paid = false, days_since_signup >= 14)
// UI: показывается in-app попап или email с ссылкой на опрос
function trackExitSurveyCompleted(
  daysSinceSignup: number,
  modulesCount: number,
  hasReachedAha: boolean,
  selectedReason: 'no_need_product_thinking' | 'no_need_vibe_coding' | 'too_expensive' | 'dont_trust_instructor' | 'not_relevant' | 'other',
  freeTextReason: string | null,
  userSegment: string
): void {
  posthog.capture('exit_survey_completed', {
    days_since_signup: daysSinceSignup,
    modules_completed: modulesCount,
    aha_reached: hasReachedAha,
    reason: selectedReason,
    reason_text: freeTextReason,
    segment: userSegment,
  })
}
```

### Prompt Health Events [ИСПРАВЛЕНИЕ 4]

```typescript
// === prompt_health_check ===
// Серверная сторона: автоматическая проверка каждого промпта 1 раз в сутки
// Проверяет, что промпт выдаёт ожидаемый формат ответа в Claude и ChatGPT
// При fail — автоматический alert команде
function trackPromptHealthCheck(
  promptName: string,
  status: 'pass' | 'fail',
  ...
  posthog.capture('prompt_health_check', {
    prompt_name: promptName, // 'idea_validator' | 'segment_generator' | ...
    status,
    response_format_valid: responseFormatValid,
    response_time_ms: responseTimeMs,
    ai_model: aiModel, // 'claude-sonnet-4-6' | 'gpt-4o' | ...
  })
}
```
...
function trackEmailSent(emailType: string, userSegment: string): void {
  posthog.capture('email_sent', {
    ...
    // 'exit_survey' — exit-опрос для free users через 14 дней [ИСПРАВЛЕНИЕ 3]
...
**Цель:** Глубокий анализ Aha-момента: кто достигает, что влияет, как оптимизировать. Включает сегмент-специфичные Aha-моменты.
...
7. **Сегмент-специфичные Aha-моменты** (PostHog Trends) [ИСПРАВЛЕНИЕ 1]
   - % пользователей, достигших `aha_segment_reached`, по `aha_type`
   - Breakdown: founders vs solopreneurs vs career vs seekers
   - Time to Aha по сегментам (медиана `time_to_aha_seconds`)
   - Корреляция: aha_segment_reached -> purchase_completed по каждому сегменту

8. **Aha Segment Funnel по типам** (PostHog Funnel) [ИСПРАВЛЕНИЕ 1]
   - Founders: signup -> exercise_completed (describe_product_jobs) -> prompt_result_saved (idea_validator) -> aha_segment_reached (founders)
   - Solopreneurs: signup -> prompt_result_saved (idea_validator) -> aha_segment_reached (solopreneurs)
   - Career Builders: signup -> exercise_completed (Module 4) -> prompts_copied >= 3 -> aha_segment_reached (career)
   - AI Skills Seekers: signup -> skill_downloaded -> aha_segment_reached (seekers)
...
### Дашборд 8: Upsell/Cross-sell Revenue [ИСПРАВЛЕНИЕ 2]

**Цель:** Полная картина дополнительного revenue от cross-sell и upsell продуктов. Оптимизация предложений.

**Графики:**

1. **Cross-sell Revenue Total** (PostHog Trends, big number)
   - Кумулятивный revenue из cross_sell_purchased.price
   - По месяцам

2. **Cross-sell Conversion by Product** (PostHog Funnel)
   - cross_sell_shown -> cross_sell_purchased
   - Breakdown по product_name
   - CR для каждого продукта (Templates Pack, Certificate, Community Pro, Prompt Pack GTM, PRD from Interview)

3. **Upsell Revenue Total** (PostHog Trends, big number)
   - Кумулятивный revenue из upsell_purchased.price
   - По месяцам

4. **Upsell Conversion by Product** (PostHog Funnel)
   - upsell_shown -> upsell_purchased
   - Breakdown по product_name (Advanced Course, Product Review, Cohort Bootcamp)

5. **Subscription MRR** (PostHog Trends)
   - subscription_started count x price — subscription_cancelled count x price
   - MRR (Monthly Recurring Revenue) из Community Pro
   - Churn rate подписок: subscription_cancelled / active subscriptions

6. **Cross-sell/Upsell by Segment** (PostHog Trends, stacked bar)
   - Revenue от cross-sell + upsell, breakdown по сегменту
   - Определяет самый "монетизируемый" сегмент после основного курса

7. **Trigger Effectiveness** (PostHog Trends)
   - CR cross_sell_shown -> cross_sell_purchased, breakdown по trigger
   - Какие триггеры (module_5_completed, course_completed, email) лучше конвертируют

8. **Total Revenue Mix** (PostHog Trends, pie chart)
   - Доля revenue: основной курс vs cross-sell vs upsell vs subscriptions
   - По месяцам (как меняется mix со временем)

### Дашборд 9: Exit Survey и диагностика неконверсии [ИСПРАВЛЕНИЕ 3]

**Цель:** Понять, ПОЧЕМУ free users не конвертируются. Диагностика главного риска (нет спроса на combo).

**Графики:**

1. **Exit Survey Response Rate** (PostHog Trends)
   - exit_survey_completed / (signup_completed WHERE is_paid = false AND days_since_signup >= 14)
   - По неделям

2. **Причины неконверсии** (PostHog Trends, pie chart)
   - exit_survey_completed, breakdown по reason
   - Показывает топ причин: no_need_product_thinking / no_need_vibe_coding / too_expensive / dont_trust_instructor / not_relevant / other

3. **Причины по сегментам** (PostHog Trends, stacked bar)
   - exit_survey_completed, breakdown по reason + segment
   - Определяет сегмент-специфичные причины отказа

4. **Причины vs Aha** (PostHog Trends)
   - exit_survey_completed WHERE aha_reached = true vs aha_reached = false
   - Breakdown по reason
   - Гипотеза: без Aha — "not_relevant", с Aha — "too_expensive"

5. **Combo Value Diagnostic** (PostHog Trends)
   - exit_survey_completed WHERE reason = 'no_need_product_thinking' vs 'no_need_vibe_coding'
   - По неделям — тренд
   - Ключевой индикатор: если "no_need_product_thinking" растёт — combo не работает, нужен pivot
...
### Exit Survey триггер [ИСПРАВЛЕНИЕ 3]

| # | Триггер | Условие | Действие | Канал | Задержка |
|---|---------|---------|----------|-------|----------|
| ES1 | Exit survey для неконвертированных | `signup_completed` + is_paid = false + days_since_signup >= 14 + НЕТ `exit_survey_completed` | "Мы заметили, что ты не продолжил курс. Помоги нам стать лучше — ответь на 1 вопрос: что остановило?" с вариантами ответа | Email + In-app | D+14 |
...
| Revenue (основной курс) | $199K-$399K | 1000 x $199-399 |
| Revenue (cross-sell/upsell) | $50K-$100K+ | cross_sell_purchased + upsell_purchased + subscriptions |
...
## 7. Журнал изменений (v1.0 -> v2.0)

Изменения внесены по результатам кросс-проверки (11-cross-verify.md):

### Исправление 1: Сегмент-специфичные Aha-моменты (Критический разрыв #1)

**Проблема:** PRD описывает 4 разных Aha-момента для 4 сегментов, аналитика трекала только один (Module 1, промпт Idea Validator). Aha для AI Skills Seekers (Module 5) и Career Builders (Module 4) не отслеживался.

**Что добавлено:**
- Новое событие `aha_segment_reached` с properties: segment, aha_type, trigger_action, time_to_aha_seconds, module
- Функция `trackAhaSegmentReached()` в коде
- Таблица триггерных действий по сегментам в секции Activation
- Графики 7 и 8 в Дашборд 6 (Aha-момент анализ) для сегмент-специфичных Aha

### Исправление 2: Cross-sell / Upsell события (Критический разрыв #2)

**Проблема:** 8 дополнительных revenue-потоков (Templates $29, Certificate $19, Community Pro $19/мес, Advanced Course $299-499, Bootcamp $699-999 и др.) полностью отсутствовали в аналитике.

**Что добавлено:**
- Новая секция событий "Cross-sell & Upsell Events" с 6 событиями: cross_sell_shown, cross_sell_purchased, upsell_shown, upsell_purchased, subscription_started, subscription_cancelled
- Функции трекинга для каждого события
- Новый Дашборд 8 "Upsell/Cross-sell Revenue" с 8 графиками
- Обновлена строка Revenue в End-to-end метриках

### Исправление 3: Exit Survey (Критический разрыв #3)

**Проблема:** Главный риск продукта (нет спроса на combo product+vibe-coding, Score 16.0) не имел механизма диагностики причин. paywall_dismissed не содержал reason. Не было exit-survey.

**Что добавлено:**
- Новое событие `exit_survey_completed` с properties: days_since_signup, modules_completed, aha_reached, reason, reason_text, segment
- Функция `trackExitSurveyCompleted()` в коде
- Триггер ES1 в секции автоматических коммуникаций
- Новый Дашборд 9 "Exit Survey и диагностика неконверсии" с 5 графиками
- Тип email 'exit_survey' в email_sent

### Исправление 4: Дополнительные события (Важные улучшения)

**Проблема:** Отсутствовали события для ключевых бизнес-вопросов: создание артефактов студентами, наличие идеи продукта, здоровье промптов.

**Что добавлено:**
- Событие `artifact_created` (type, module, tool_used, segment) и функция `trackArtifactCreated()`
- Событие `onboarding_has_idea` (has_idea, segment) и функция `trackOnboardingHasIdea()`
- Событие `prompt_health_check` (prompt_name, status, response_format_valid, response_time_ms, ai_model) и функция `trackPromptHealthCheck()`
- Обновлён posthog.identify: добавлены свойства has_product_idea, artifacts_created_count, cross_sell_purchased_count
