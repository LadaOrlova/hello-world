---
## Инструкция от Вани Замесина

> *[Это поле заполняет автор после прочтения черновика]*
> *[Напишите здесь, что нужно изменить на этапе редактуры]*

---

# Judgment, not features: what 20 vibe-coded products taught me

> **Статус:** Черновик v4 (style-only rewrite)
> **Модель:** claude-opus-4-6
> **Дата:** 2026-03-11
> **Целевое издание:** Mind the Product — https://www.mindtheproduct.com
> **Язык:** English
> **Слов:** ~1100
> **Тезисов:** 5 (A1, A2, A3, A4, A6)
> **Режим:** Style-only rewrite (narrative-architect + style-researcher + writer)
> **Что изменено vs оригинала:** Только хук, переходы между секциями, close, ритм абзацев. Все кейсы, ссылки, логика и структура — из Ваниного драфта v2.

Over the past three months, I built 20 products with vibe coding. They've already generated $100K in revenue. But the real lesson wasn't about speed — it was about everything speed doesn't solve. Here's what happens to the PM role when building costs almost nothing.

## The builder era is here

Boris Cherny, Head of Claude Code at Anthropic, says that [95% of his code is AI-written](https://www.lennysnewsletter.com/p/head-of-claude-code-what-happens), with +200% productivity per engineer. A quarter of Y Combinator's W25 batch ships with [95% AI-generated codebases](https://techcrunch.com/2025/03/06/a-quarter-of-startups-in-ycs-current-cohort-have-codebases-that-are-almost-entirely-ai-generated/). Collins Dictionary named "vibe coding" [Word of the Year 2025](https://www.theguardian.com/technology/2025/nov/06/vibe-coding-collins-dictionary-word-of-the-year-2025). That's three signals from three different worlds — engineering, startups, culture — all pointing the same direction.

Marc Andreessen calls it a ["Mexican standoff" between PM, engineering, and design](https://www.lennysnewsletter.com/p/marc-andreessen-the-real-ai-boom) — each believes they can do the other two roles with AI, and "they're actually all kind of correct." LinkedIn formalized this in December 2025, scrapping its APM program and launching ["Associate Product Builder"](https://www.lennysnewsletter.com/p/why-linkedin-is-replacing-pms) — new hires learn coding, design, and PM together in cross-trained pods.

Here's the reframe that matters: a full-stack builder isn't a PM who just learned to code. It's a PM who can test dozens of hypotheses across segments, value propositions, activation, and business models — and maintain production quality through tests, reviews, and economics.

Writing PRDs, creating mockups, moving Jira tasks — that work is disappearing in front of our eyes. You level up from pushing tasks to owning the entire value chain.

But here's what nobody tells you about the builder era: building faster is the easy part. The hard part is knowing what's worth building.

## Speed without judgment is the feature mill on steroids

When AI crushes execution cost to near zero, the bottleneck moves upstream — to business judgment. You're diagnosing where in the causal chain the growth actually sits:

- Market segment
- Jobs to be done
- Strategy
- Value proposition
- Unit economics
- Acquisition channels

You're not optimizing features. You're bringing growth to the business.

Marty Cagan [puts it bluntly](https://www.svpg.com/product-coaching-and-ai/): AI makes "PM theater" — PRDs, roadmaps, user stories — cheap and replaceable by agents. The PM who moves tickets is exactly who gets replaced. Ravi Mehta frames it as a [shift from craft to judgment](https://www.atlassian.com/blog/artificial-intelligence/shift-from-craft-to-judgement-ai): when anyone can produce something polished, knowing what to create and why is the scarce asset.

Without judgment, vibe coding is the feature mill on steroids. Ninety percent of resources already go to features that never find a market. AI makes that 10x faster.

So if judgment is the asset and building is the tool — what does the day-to-day actually look like?

## From PRDs to hypothesis pipelines

When AI collapses the cost of building and routine operations, you can test 10-100x more hypotheses per sprint. The paradigm flips: you're not launching products — you're buying knowledge.

Every prototype is a bet. And the bottleneck shifts from execution to the pipeline itself: business goal, causal graph of growth mechanics, research, jobs to be done, value hypotheses, riskiest assumption test, build, measure, repeat. The PM becomes an operator of a hypothesis factory — designing it, feeding it, optimizing its throughput. Each step can be delegated to an AI agent. And increasingly, it is.

Boris Cherny describes the frontier: "Claude is starting to come up with ideas. Looking through feedback, bug reports, telemetry." The agent doesn't just execute — it proposes.

Shreyas Doshi frames the scale: 3-4 person startups now match the impact of 30-50 person teams. But this advantage only works if delegation follows risk. AI agents are less competent employees — hand them low-risk tasks (A/B tests, data analysis, draft copy) and keep the high-risk decisions for yourself: which segment to enter, what to price, where the ethical lines sit.

But let's be honest about what's not ready yet.

## The full cycle isn't here yet

You can prototype in an afternoon. You can't ship to production in an afternoon. The critical chain breaks at security, DevOps, and compliance — the slowest link determines your actual speed.

AI makes creation cheap, but each component you add multiplies the probability of failure. The full cycle is 12-24 months away. Until then: kill criteria, reversibility, and circuit breakers.

These gaps will close. The question is who'll be ready when they do — and who'll be left behind.

## The split is already happening

The PM market is fracturing K-shaped. The WEF [captures the paradox](https://www.weforum.org/stories/2025/10/ai-s-new-dual-workforce-challenge-balancing-overcapacity-and-talent-shortages/): 94% of leaders face critical skill shortages in AI-ready roles, yet 92% report up to 20% workforce overcapacity — expected to hit 30% by 2028. Read that again: shortage and surplus at the same time. The split isn't coming. It's here.

The forces are structural. Builder PMs feel strong pull forward — the value is enormous, the pain of the old model obvious, barriers low. Traditional PMs feel equally strong pushback — habit deeply embedded, fear of coding real. People overestimate their readiness to change by a factor of three.

This is the innovator's dilemma playing out in real time. Companies don't get disrupted because they work badly — they get disrupted because they work well enough to resist change. The PM who operates at local optimizations gets replaced. The PM who works at the global level — which market, which segment, when to pivot — becomes irreplaceable.

The reframe isn't "adapt or die." It's this: build the environment — training, access, psychological safety — that lets people adapt. Bifurcation isn't about talent. It's about architecture.

And it starts with what you do this week.

## Where to start

1. **Build one prototype this week** — not to ship, but to learn what your judgment tells you when testing costs nothing.
2. **Find the growth constraint in your business model** — is it activation, positioning, segment choice, or unit economics?
3. **Stress-test one backlog item:** "If this assumption is wrong, what do we lose?" Rank by consequence, not effort.
4. **Talk to one customer this week.** No AI can replicate what you learn from watching someone struggle with your product in real time.

Pick one. Do it before Friday. That's the muscle that matters now.
